...
func update_arrows():
	...
	for cell in all_arrows:
		...
		all_arrows[cell].rotate(flow_field.get_exact_cell_direction(cell).angle() + deg2rad(90) )
		flow_field_result.append(flow_field.get_exact_cell_direction(cell))
...
func measure_access_speed():
	...
	while (i < 10000):
		i+= 1
		var direction = flow_field.get_exact_cell_direction(Vector2(0,0))
...
func _process(delta):
	...
	if Input.is_action_just_pressed("select_cell"):
		var mous_position = get_global_mouse_position()
		var cell:Vector2 = mous_position / flow_field.cell_size
		print(str(flow_field.get_exact_cell_direction(cell)) + " , " + str(flow_field.get_process_number_of_cell(cell)))
